...
case_tweets <- senator_tweets %>% 
  ...
  mutate(average_sen_approval = 
           mean(prelim_thoughts))

west <- c("CA", "WA", "OR", "NV", "MT", "ID",
          "WY", "CO", "NM", "AZ", "AK", "HI")

south <- c("TX", "OK", "LA", "MO", "MS", "AL", "GA", "FL", "KY",
           "TN", "SC", "NC")

midwest <- c("ND", "SD", "NE", "KS", "MN", "WI", "MI", "OH", "IN", "IO",
             "OH")

mid_atlantic <- c("NY", "VA", "PA", "MD", "DE")

northeast <- c("CN", "MA", "NH", "ME", "RI")

region <- as.tibble(c(west, midwest, south, northeast, mid_atlantic))
...
public_averages %>% 
  ...
  geom_text(aes(label = state), position = "dodge", size = 3) 
```
